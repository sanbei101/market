...
const callGenerateTestData = `-- name: CallGenerateTestData :exec
...
type CallGenerateTestDataParams struct {
	CategoryCount  int32
	SpuPerCategory int32
	SkuPerSpu      int32
}

func (q *Queries) CallGenerateTestData(ctx context.Context, arg CallGenerateTestDataParams) error {
	_, err := q.db.Exec(ctx, callGenerateTestData, arg.CategoryCount, arg.SpuPerCategory, arg.SkuPerSpu)
	return err
}

const generateTestData = `-- name: GenerateTestData :exec
...
func (q *Queries) GenerateTestData(ctx context.Context) error {
	_, err := q.db.Exec(ctx, generateTestData)
	return err
}
